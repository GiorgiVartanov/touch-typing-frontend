...
const Match = () => {
    const {uid, username, match_id, matches, LeaveMatch, ModifyMatch, NotifyFinish, match_finished} = usePlayStore() // Update variables
    ...
    useEffect(()=>{
        if(match_finished===true)
            navigate("../match/"+match_id) // Navigate to the finished match
        return () => {
            if(location.pathname!=="/play/"+match_id && match_id){ // Update pathname
    ...
    if(match_id === undefined || matches[match_id] === undefined || (matches[match_id].players[uid] === undefined && matches[match_id].spectators[uid] === undefined))     
    ...
    return (
        <div className="page match"> {/* Update class name */}
            { match.has_started !== true ?
                <>
                    <h1>
                        Waiting for users to join: {match.active_players}/{match.user_limit} {/* Update variables */}
                    </h1>
                    <Button
                ...
                <>
                    ...
                    }
                    {
                        Object.keys(match.players).map((uid, ind)=>{
                            return <div key={ind} className={(match.players[uid].username===username?"active-background":"")+ " list"}>
                                <h1 key={1}>
                                    { match.players[uid].username }
                                </h1>
                                <h1 key={2}>
                                    { match.players[uid].WPM !== -1 ?
                                        match.players[uid].has_finished ? " WPM: " : " Progress: "
                                        :" Disconnected: " }
